...
def Irf(params,f_RF):
    ...
    f_FSR = params['FSR'].value  # modulation multiplication
    T = params['T'].value
    A0 = params['A0'].value
    f0 = params['f0'].value
    Q = params['Q'].value
    ...
    #S_Irf = sin(phi_f/2)*(1 + 3*R + R**2 - (1+2*cos(phi_f))*R)
    #S_Qrf = cos(phi_f/2)*(1 - 3*R + R**2 - (1-2*cos(phi_f))*R)
...
def e2(params,f_RF,rII,rQI,rQQ,rIQ):
    ...
    FSR = params['FSR'].value  # modulation multiplication
    f0 = params['f0'].value
    Q = params['Q'].value
...
audio_freq = 303.0
data = genfromtxt('data/REFL45_303_18Dec.txt')
print shape(data)
...
params.add('f0',value=9.185e6*5,vary=True, min=9e6*5,max=9.5e6*5)
params.add('theta0',value=0.0, vary=False, min=0.0, max=2*pi)
params.add('Q',value=60.0,vary=True,min=10, max=200)
...
print
print 'FSR [Hz] =', FSR
print '--> Change in RF phase across frequency range [deg] =', theta_shift
print
...
#             fancybox=True,loc='upper center', bbox_to_anchor=(0.47, 1.3),ncol=2, fontsize=16)

pylab.title('REFL45 303Hz')
...
pylab.yticks(array([-0.02, 0.0, 0.02]))

pylab.xticks(fontsize=18)
pylab.yticks(fontsize=18)

pylab.savefig('plots/REFL45_303_full.pdf',bbox_inches='tight')
...
pylab.savefig('plots/global_FSR_zoom.pdf')
